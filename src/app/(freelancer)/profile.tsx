...
// Freelancer-flavoured labels for the shared OwnerTier system
const TIER_CONFIG: Record<OwnerTier, { label: string; color: string; xpNeeded: number }> = {
  client:   { label: 'Newcomer',  color: '#6B7280', xpNeeded: 50 },
...
const BADGE_CONFIG: Record<OwnerBadge, { icon: string; label: string }> = {
  active_client:  { icon: '🏄', label: 'Active Proprietor' },
  big_spender:    { icon: '💰', label: 'Big Spender'       },
  great_reviewer: { icon: '⭐', label: 'Great Reviewer'    },
};
  ...
  return (
    <View style={s.root}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.hero}>
          <LinearGradient
            colors={[colors.brand.secondary + '25', colors.brand.neon + '08', 'transparent']}
          ...
          {!user ? (
            ...
            <SafeAreaView edges={['top']} style={s.heroContent}>
              ...
              <TouchableOpacity style={s.pubkeyRow} onPress={() => openInExplorer(pubkey)} activeOpacity={0.7}>
                ...
                <TouchableOpacity hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} onPress={copyAddress} activeOpacity={0.7}>
                  <Ionicons
        ...
        <View style={s.body}>
          ...
          <View style={s.section}>
            <Text style={s.sectionTitle}>Agent Details</Text>
            <View style={s.infoGroup}>
              <View style={s.infoRow}>
                <View style={s.infoIcon}>
                  <Ionicons name="person-outline" size={18} color={colors.brand.secondary} />
                </View>
                <View style={{ flex: 1 }}>
              ...
              <View style={s.infoRow}>
                <View style={s.infoIcon}>
                  <Ionicons name="body-outline" size={18} color={colors.brand.secondary} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={s.infoLabel}>Primary Role</Text>
                  <Text style={s.infoValue}>Agent Proprietor</Text>
                </View>
              </View>
              <View style={s.divider} />
              <View style={s.infoRow}>
                <View style={s.infoIcon}>
                  <Ionicons name="calendar-outline" size={18} color={colors.brand.secondary} />
                </View>
                <View style={{ flex: 1 }}>
          ...
          {/* ── Reputation ─────────────────────────────────── */}
          {ownerStats && (() => {
            const cfg = TIER_CONFIG[ownerStats.tier ?? 'client'] ?? TIER_CONFIG.client;
            const xp = ownerStats.xp_points ?? 0;
            const xpPct = Math.min(100, (xp / cfg.xpNeeded) * 100);
            return (
              <View style={s.section}>
                ...
                <GlassCard intensity="low" border style={s.repCard}>
                  ...
                  <View style={s.repTierRow}>
                    <View style={[s.repTierPill, { borderColor: cfg.color + '50', backgroundColor: cfg.color + '18' }]}>
                  ...
                  {/* XP progress bar */}
                  {(ownerStats.tier ?? 'client') !== 'platinum' && (
                    <View style={{ gap: 4 }}>
                      ...
                      <Text style={s.repBarHint}>{xp} / {cfg.xpNeeded} XP to next tier</Text>
                    </View>
                  )}
                  ...
                  <View style={s.repStatsRow}>
                    ...
                    <View style={s.repStat}>
                      <Text style={s.repStatValue}>{ownerStats.ratings_given ?? 0}</Text>
                      <Text style={s.repStatLabel}>Rated</Text>
                    </View>
                    <View style={s.repStatDivider} />
                    <View style={s.repStat}>
                      ...
                      {(ownerStats.badges ?? []).map((b) => {
                        ...
                        return (
                          <View key={b} style={s.repBadge}>
                            <Text style={s.repBadgeIcon}>{bc.icon}</Text>
          ...
          >
            <LinearGradient
              colors={['transparent', colors.states.error + '12']}
              style={StyleSheet.absoluteFill}
            />
            {isLoggingOut ? (
              <>
...
const s = StyleSheet.create({
  ...
  avatarGlow: {
    ...
    borderRadius: 52, backgroundColor: colors.brand.secondary, opacity: 0.18,
  },
  avatar: {
    width: 96, height: 96, borderRadius: 48,
    backgroundColor: colors.surface.elevated2,
    borderWidth: 2, borderColor: colors.brand.secondary,
    alignItems: 'center', justifyContent: 'center',
  },
  avatarLetter: { fontSize: 34, fontWeight: '900', color: colors.brand.secondary },

  nameText:   { color: colors.text.primary, marginTop: 14 },
  handleText: {
    fontSize: 14, fontWeight: '600', color: colors.brand.neon,
    marginTop: 2, letterSpacing: 0.3,
  },
  pubkeyRow: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    marginTop: 12, paddingHorizontal: 14, paddingVertical: 7,
    backgroundColor: colors.surface.base, borderRadius: 20,
  ...
  dollar:      { fontSize: 18, fontWeight: '900', color: colors.brand.primary },
  ...
  infoLabel: { fontSize: 10, fontWeight: '700', color: colors.text.muted, textTransform: 'uppercase', letterSpacing: 0.5 },
  ...
  repXpBadge: {
    ...
    backgroundColor: colors.brand.neon + '18', borderRadius: 12,
    paddingHorizontal: 10, paddingVertical: 5,
    borderWidth: 1, borderColor: colors.brand.neon + '35',
  },
  ...
  repStatLabel: { fontSize: 10, fontWeight: '700', color: colors.text.muted, textTransform: 'uppercase', letterSpacing: 0.8 },
  ...
  logoutBtn: {
    ...
    borderWidth: 1, borderColor: colors.states.error + '35',
    backgroundColor: colors.background.secondary, overflow: 'hidden',
  },
